...
- «Выполненные цели» (goal_completions) = достижения всех целей Метрики, привязанных к счётчику. Это не заявки и не продажи — данных о реальных заявках, продажах и выручке из 1С пока нет, не делай о них выводов и не путай goal_completions с лидами.
- CR и CPL считаются от выполненных целей (goal_completions).
- Пиши по-русски, коротко, с конкретными числами из данных. Никаких общих фраз вроде «продолжайте оптимизировать».
- verdict: good — выполненные цели растут или CPL ниже среднего при заметном бюджете; ok — норма; warn — CPL заметно выше среднего, падение целей/CTR, рост CPC; bad — расход без результата или резкое ухудшение.
...
def agg(rows):
    ...
    return {
        ...
        "goal_completions": a["conv"], "cr": d(a["conv"], a["clicks"]) if a["conv"] is not None else None,
...
def extra_context():
    ...
    if sl:
        ...
        ctx["geo"] = {
            "top_regions": [{"region": r[0], "cost": r[3], "goal_completions": r[4]} for r in geo[:10]],
        ...
        ctx["placements"] = [{"placement": r[0], "cost": r[4], "goal_completions": r[5]}
                             for r in sorted(pl, key=lambda r: -r[4])[:10]]
        ctx["audience"] = [{"gender": r[0], "age": r[1], "cost": r[4], "goal_completions": r[5]}
    ...
    if kw:
        ...
        fmt = lambda r: {"keyword": r[1], "cid": r[2], "impressions": r[5], "clicks": r[6],
                         "cost": r[7], "goal_completions": r[8], "traffic_volume": r[9]}
    ...
    if q:
        ...
        ctx["queries"] = {"top_cost_no_result": [{"query": r[0], "cid": r[1], "clicks": r[5], "cost": r[6]}
                                                 for r in waste],
                          "total_cost_no_result": round(sum(r[6] for r in rows if (r[7] or 0) == 0))}
    return ctx
